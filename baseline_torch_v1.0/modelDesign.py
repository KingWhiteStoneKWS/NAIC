...
class ANN_TypeI(nn.Module): ## Neural Network (input:location, output: the estimated CSI)
    def __init__(self, in_features):
        ...
        self.x_con1 = nn.Linear(3, 128) 
        #leaky_relu
        self.x_con3 = nn.ConvTranspose2d(8, 256, 4, 2, 1)
        self.x_con4 = nn.BatchNorm2d(256)
        #leaky_relu            
        self.x_con5 = nn.ConvTranspose2d(256, 128, 4, 2, 1)
        self.x_con6 = nn.BatchNorm2d(128)
        #leaky_relu
        self.x_con7 = nn.ConvTranspose2d(128, 64, 4, 2, 1)
        self.x_con8 = nn.BatchNorm2d(64)
        #leaky_relu
        self.x_con9 = nn.ConvTranspose2d(64, 8, 4, 2, 1)
        #tanh

    def forward(self, p):
        x_con = self.x_con1(p)
        x_con = torch.reshape(x_con,(-1,8,4,4))
        x_con = F.leaky_relu(x_con,0.2)
        ...
        x_con = F.leaky_relu(x_con,0.2)
        x_con = self.x_con7(x_con)
        x_con = self.x_con8(x_con)
        x_con = F.leaky_relu(x_con,0.2)
        x_con = self.x_con9(x_con)
